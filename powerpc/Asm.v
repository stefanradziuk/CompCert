...
Inductive instruction : Type :=
  ...
  | Pdcbt: int_compcert -> ireg -> ireg -> instruction                 (**r data cache block touch *)
  | Pdcbtst: int_compcert -> ireg -> ireg -> instruction               (**r data cache block touch *)
  | Pdcbtls: int_compcert -> ireg -> ireg -> instruction               (**r data cache block touch and lock *)
  ...
  | Pfcti: ireg -> freg -> instruction                        (**r float-to-signed-int_compcert conversion, round towards 0 (pseudo) *)
  | Pfctid: ireg -> freg -> instruction                       (**r float-to-signed-int_compcert conversion, round towards 0 (pseudo, PPC64) *)
  | Pfctidz: freg -> freg -> instruction                      (**r float-to-signed-long conversion, round towards 0 (PPC64) *)
  | Pfctiw: freg -> freg -> instruction                       (**r float-to-signed-int_compcert conversion, round by default *)
  | Pfctiwz: freg -> freg -> instruction                      (**r float-to-signed-int_compcert conversion, round towards 0 *)
  ...
  | Picbtls: int_compcert -> ireg -> ireg -> instruction               (**r instruction cache block touch and lock set *)
  | Plbz: ireg -> constant -> ireg -> instruction             (**r load 8-bit unsigned int_compcert *)
  | Plbzx: ireg -> ireg -> ireg -> instruction                (**r same, with 2 index regs *)
  | Pld: ireg -> constant -> ireg -> instruction              (**r load 64-bit int_compcert (PPC64) *)
  | Pldx: ireg -> ireg -> ireg -> instruction                 (**r same, with 2 index regs *)
  | Pld_a: ireg -> constant -> ireg -> instruction            (**r load 64-bit quantity to int_compcert reg (PPC64) *)
  | Pldbrx: ireg -> ireg -> ireg -> instruction               (**r load 64-bit int_compcert and reverse endianness (PPC64) *)
  ...
  | Plha: ireg -> constant -> ireg -> instruction             (**r load 16-bit signed int_compcert *)
  | Plhax: ireg -> ireg -> ireg -> instruction                (**r same, with 2 index regs *)
  | Plhbrx: ireg -> ireg -> ireg -> instruction               (**r load 16-bit int_compcert and reverse endianness *)
  | Plhz: ireg -> constant -> ireg -> instruction             (**r load 16-bit unsigned int_compcert *)
  | Plhzx: ireg -> ireg -> ireg -> instruction                (**r same, with 2 index regs *)
  | Pldi: ireg -> int64 -> instruction                        (**r load 64-bit int_compcert constant (PPC64) *)
  ...
  | Plwz: ireg -> constant -> ireg -> instruction             (**r load 32-bit int_compcert *)
  | Plwzu: ireg -> constant -> ireg -> instruction            (**r load 32-bit int_compcert with update *)
  | Plwzx: ireg -> ireg -> ireg -> instruction                (**r same, with 2 index regs *)
  | Plwz_a: ireg -> constant -> ireg -> instruction           (**r load 32-bit quantity to int_compcert reg *)
  | Plwzx_a: ireg -> ireg -> ireg -> instruction              (**r same, with 2 index regs *)
  | Plwarx: ireg -> ireg -> ireg -> instruction               (**r load with reservation *)
  | Plwbrx: ireg -> ireg -> ireg -> instruction               (**r load 32-bit int_compcert and reverse endianness *)
  | Pmbar: int_compcert -> instruction                                 (**r memory barrier *)
  ...
  | Pmfspr: ireg -> int_compcert -> instruction                        (**r move from special register *)
  | Pmtspr: int_compcert -> ireg -> instruction                        (**r move to special register *)
  ...
  | Prldicl: ireg -> ireg -> int_compcert -> int_compcert -> instruction        (**r rotate and mask left (PPC64) *)
  | Prldinm: ireg -> ireg -> int_compcert -> int64 -> instruction      (**r rotate and mask (PPC64) *)
  | Prldimi: ireg -> ireg -> int_compcert -> int64 -> instruction      (**r rotate and insert (PPC64) *)
  | Prlwinm: ireg -> ireg -> int_compcert -> int_compcert -> instruction        (**r rotate and mask *)
  | Prlwimi: ireg -> ireg -> int_compcert -> int_compcert -> instruction        (**r rotate and insert *)
  ...
  | Psradi: ireg -> ireg -> int_compcert -> instruction                (**r shift right signed immediate 64 bits (PPC64) *)
  | Psraw: ireg -> ireg -> ireg -> instruction                (**r shift right signed *)
  | Psrawi: ireg -> ireg -> int_compcert -> instruction                (**r shift right signed immediate *)
  ...
  | Pstb: ireg -> constant -> ireg -> instruction             (**r store 8-bit int_compcert *)
  ...
  | Pstdbrx: ireg -> ireg -> ireg -> instruction              (**r store 64-bit int_compcert with reverse endianness (PPC64) *)
  | Pstdu: ireg -> constant -> ireg -> instruction            (**r store 64-bit integer with update (PPC64) *)
  | Pstd_a: ireg -> constant -> ireg -> instruction           (**r store 64-bit quantity from int_compcert reg (PPC64) *)
  ...
  | Psth: ireg -> constant -> ireg -> instruction             (**r store 16-bit int_compcert *)
  | Psthx: ireg -> ireg -> ireg -> instruction                (**r same, with 2 index regs *)
  | Psthbrx: ireg -> ireg -> ireg -> instruction              (**r store 16-bit int_compcert with reverse endianness *)
  | Pstw: ireg -> constant -> ireg -> instruction             (**r store 32-bit int_compcert *)
  | Pstwu: ireg -> constant -> ireg -> instruction            (**r store 32-bit int_compcert with update *)
  ...
  | Pstw_a: ireg -> constant -> ireg -> instruction           (**r store 32-bit quantity from int_compcert reg *)
  | Pstwx_a: ireg -> ireg -> ireg -> instruction              (**r same, with 2 index regs *)
  | Pstwbrx: ireg -> ireg -> ireg -> instruction              (**r store 32-bit int_compcert with reverse endianness *)
  ...
  | Pcfi_rel_offset: int_compcert -> instruction.                      (**r .cfi_rel_offset lr debug directive *)
...
- [Pfcti]: convert a float to a signed integer.  This requires a transfer
  via memory of a 32-bit integer from a float register to an int_compcert register.
  Expands to:
<<
        fctiwz  f13, rsrc
        stfdu   f13, -8(r1)
